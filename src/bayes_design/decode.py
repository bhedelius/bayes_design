...
def decode_combinations(
    prob_model, struct, seq, unmasked_seq, decode_order, fixed_position_mask, from_scratch, exclude_aa=None, **kwargs
):
    ...
    unfixed_positions = [i for i, mask in enumerate(fixed_position_mask) if not mask]
    ...
    for n_positions in range(2, max_n_positions + 1):
        ...
        for i, combination in enumerate(combinations):
            ...
            orig_log_prob = metric_dict["log_prob"](
                seq=unmasked_seq,
            ...
            decoded_seq, new_log_prob = greedy_decode(
                prob_model=prob_model,
                struct=struct,
                seq="".join(masked_seq),
                decode_order=decode_order,
...
def max_prob_decode(
    ...
    **kwargs,
    ...
    for i in range(n_decoded):
        ...
        probs = prob_model(
            seq=["".join(current_seq)] * len(token_to_decode),
            struct=struct,
            decode_order=decode_order,
...
def greedy_decode(
    ...
    **kwargs,
    ...
    current_seq = seq
    log_probs = []
    for _i, idx in enumerate(decode_order):
        if fixed_position_mask[idx]:
        ...
        probs = prob_model(
            ...
            struct=struct,
            decode_order=decode_order,
            token_to_decode=torch.tensor([idx]),
            mask_type=mask_type,
        )
        for aa in exclude_aa:
...
def sample_decode(
    prob_model, struct, seq, decode_order, fixed_position_mask, from_scratch, exclude_aa, temperature=1.0, **kwargs
):
...
def beam_decode(
    prob_model, struct, seq, decode_order, fixed_position_mask, exclude_aa, from_scratch, n_beams, **kwargs
):
    ...
    n_concurrent_seqs = int(196 / 32510 * (0.9 * gpu_mem / 1e6))
    ...
    for j, decode_idx in enumerate(decode_order):
        ...
        if fixed_position_mask[decode_idx]:
            continue
        ...
        for i in range(0, len(seqs), n_concurrent_seqs):
            probs = prob_model.forward(
                seq=seqs[i : i + n_concurrent_seqs],
                struct=struct,
                decode_order=decode_order,
                token_to_decode=torch.tensor([decode_idx]).expand(len(seqs[i : i + n_concurrent_seqs])),
                mask_type=mask_type,
            )
...
decode_order_dict: dict[str, Callable[..., Any]] = {
    "proximity": get_proximity_decode_order,
    "reverse_proximity": get_reverse_proximity_decode_order,
    "random": get_random_decode_order,
    "n_to_c": get_n_to_c_decode_order,
}
decode_algorithm_dict: dict[str, Callable[..., Any]] = {
    "greedy": greedy_decode,
    "beam": beam_decode,
    "sample": sample_decode,
    "random": random_decode,
    "max_prob_decode": max_prob_decode,
    "combinations": decode_combinations,
}
