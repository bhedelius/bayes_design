"""Command-line entry point for experiments (``bayes-design-experiment``).

Example commands are in ``experiments.md``.
"""
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run BayesDesign experiments.")
    subparsers = parser.add_subparsers(help="Which experiment to run")
    ...
    compare_struct_correlation_parser.add_argument(
        "--perc_residues_to_mutate",
        help="The percentage of residues to mutate in each variant",
        default=0.1,
        type=float,
    )
    compare_struct_correlation_parser.add_argument(
...
def parse_seq(args: argparse.Namespace) -> argparse.Namespace:
    """Populate ``args.sequences`` from ``--sequences_path`` if needed."""
    if args.sequences is not None:
        ...
        with open(args.sequences_path) as f:
            for line in f:
                if line[0] == ">":
                    continue
                args.sequences.append(line.strip())
    return args
