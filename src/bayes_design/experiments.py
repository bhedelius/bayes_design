...
def compare_struct_correlation(args):
    ...
    # Decode order defines the order in which the masked positions are predicted
    decode_order_dict[args.decode_order](masked_seq)
    ...
    log_p_protein_mpnn = []
    log_p_trRosetta = []
    sequence_variants = generate_sequence_variants(
        orig_seq=sequence, num_variants=args.num_variants, perc_residues_to_mutate=args.perc_residues_to_mutate
    )
    for seq in sequence_variants:
        log_p_protein_mpnn.append(
            metric_dict["log_prob"](
                seq=seq,
                prob_model=protein_mpnn,
...
def make_pssm(args):
    ...
    for _i, designed_seq in enumerate(designed_seqs):
...
def viz_probs(args):
    ...
    probs = compare_probs(
        struct_to_seq_model=protein_mpnn,
        seq_model=xlnet,
        struct=structure,
        seq=seq,
        decode_order=decode_order,
    ...
    for i, idx in enumerate(indices):
        ...
        p_seq_struct, p_seq, p_struct_seq = (
            p_seq_struct[0].detach().cpu().numpy(),
            p_seq[0].detach().cpu().numpy(),
            p_struct_seq[0].detach().cpu().numpy(),
        )
...
def compare_probs(
    struct_to_seq_model, seq_model, struct, seq, decode_order, bayes_balance_factor=0.0, mask_type="bidirectional_mlm"
):
    ...
    for i in range(len(seq)):
        p_seq_struct = struct_to_seq_model(
            seq=[seq], struct=struct, decode_order=decode_order, token_to_decode=torch.tensor([i]), mask_type=mask_type
        ).clone()
        p_seq = seq_model(
            [seq], decode_order=decode_order, token_to_decode=torch.tensor([i]), mask_type=mask_type
        ).clone()
